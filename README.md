...
The scanner has been extended to support both human-readable and machine-readable results.

Current output structure:
- `summary.txt` : human-readable summary
- `scan.log` : full execution log
- `artifacts/` : evidence files generated by each module
- `results.ndjson` : structured JSON-line output for downstream processing

This makes the project more suitable for repeatable security checks, audit evidence collection, and future automation use cases.

---

### Main script enhancements
The main scanner script was updated to generate structured runtime results.

Key changes:
- added `NDJSON="$OUT_DIR/results.ndjson"`
- initialize `results.ndjson` during execution
- added `emit_ndjson()` helper function
- added keyed wrapper functions:
  - `good_k`
  - `warn_k`
  - `vuln_k`
- exported required variables/functions so child modules can emit structured records

This allows each module to produce:
1. operator-friendly terminal/summary output
2. structured JSON-line records for later reuse

---

### Structured check naming introduced
To improve result consistency, selected modules now emit explicit `check` names instead of relying only on generic summary text.
...
Purpose:
- detect root SSH exposure
- detect password-based SSH authentication status
- detect empty password login risk

#### `95_patch_security.sh`
Structured checks added:
- `security_updates`
- `reboot_required`

Purpose:
- detect pending security-related package updates
- detect reboot requirement after package/security changes

#### `35_account_basic.sh`
Structured account-related checks were aligned for:
- `uid0_accounts`
- `empty_password_accounts`
- `sudo_nopasswd`

Purpose:
- identify unexpected UID 0 accounts
- identify empty password accounts
- identify risky `sudo NOPASSWD` configuration

---

### Why this update matters
Originally, the project focused mainly on text-based security summaries and evidence files.

With these updates, the scanner now supports:
- evidence-friendly baseline security checks
- reusable structured output
- more consistent module-level security findings
- easier future expansion for centralized processing or automation workflows

This is an important step toward turning the project from a plain text hardening script into a more structured Linux security assessment tool.

---

### Example structured result
```json
{"ts":"2026-04-01T01:22:46+0000","host":"vm-int-security-ops-linux","os_type":"linux","scanner":"secscan_rhel9_kisa","check":"root_ssh","result":"warning","detail":"Root key login is allowed (without-password). If policy requires PermitRootLogin no, this should be reviewed."}
